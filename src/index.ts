...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Statistics Denmark');
}
...
const tools: McpToolExport['tools'] = [
  {
    name: 'denmark_population',
    description:
      "Denmark's total population in ONE call, from Statistics Denmark (Danmarks Statistik, table FOLK1A), " +
      "keyless — plus the recent quarterly trend. PREFER for \"what is Denmark's population\", \"how many " +
      "people live in Denmark\", \"Danish population growth\", \"Denmark population according to official " +
      "statistics\". Counts are registered residents at the FIRST DAY of each quarter, so \"2026Q2\" is a " +
      "1 April snapshot, not an average over the quarter. Use list_tables / get_data for breakdowns by " +
      "region, age, sex or marital status.",
    inputSchema: {
      type: 'object',
      properties: {
        quarters: { type: 'number', description: 'How many recent quarters to include in the trend (1-40, default 8). The headline figure is always the newest.' },
      },
    },
  },
  {
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  switch (name) {
    ...
    case 'denmark_population': {
      const quarters = Math.min(40, Math.max(1, Number(args.quarters) || 8));
      // Whole country, both sexes, all ages, all marital statuses — the four
      // dimensions FOLK1A requires. Omitting any of them makes DST return the
      // fully crossed table (hundreds of thousands of cells) instead of a total.
      const qs = new URLSearchParams({
        lang: 'en', 'OMRÅDE': '000', 'KØN': 'TOT', ALDER: 'IALT', CIVILSTAND: 'TOT', Tid: '*',
      });
      const res = await pwFetch(`${BASE}/data/FOLK1A/JSONSTAT?${qs.toString()}`, {
      ...
      const body = (await res.json()) as {
        ...
        value?: number[]; dimension?: Record<string, unknown>;
      };
      const ds = body.dataset ?? body;
      const values = ds.value ?? [];
      const idx = ((ds.dimension as Record<string, { category?: { index?: Record<string, number> } }> | undefined)
        ?.Tid?.category?.index) ?? {};
      const ordered = Object.entries(idx).sort((a, b) => a[1] - b[1]);
      const points = ordered
        .map(([period, i]) => ({ period: period.replace('K', 'Q'), population: values[i] ?? null }))
        .filter((p) => typeof p.population === 'number');
      if (!points.length) throw new Error('Statistics Denmark returned no population values for FOLK1A.');
      const recent = points.slice(-quarters).reverse();
      const latest = recent[0]!;
      const yearAgo = points.find((p) => p.period === shiftQuarters(latest.period, -4));
      return {
        country: 'Denmark',
        as_of: latest.period,
        population: latest.population,
        basis: 'registered residents at the FIRST DAY of the quarter — "2026Q2" is a 1 April snapshot, not a quarterly average',
        change_year_on_year: yearAgo && typeof yearAgo.population === 'number'
          ? (latest.population as number) - yearAgo.population
          : null,
        series: recent,
        table_id: 'FOLK1A',
        source: 'Statistics Denmark (Danmarks Statistik)',
        interpretation:
          'Counts people registered in the Danish CPR system, which is a residence register rather than a ' +
          'census estimate: asylum seekers without a CPR number and unregistered residents are absent, and ' +
          'people who emigrate without deregistering linger. Greenland and the Faroe Islands are NOT included.',
      };
    }
    case 'get_data': {
      const tableId = reqStr(args, 'tableId', '"FOLK1C"');
      ...
      const res = await pwFetch(`${BASE}/data/${encodeURIComponent(tableId)}/${format}?${qs.toString()}`, {
        headers: { Accept: 'application/json', 'User-Agent': UA },
      });
      if (!res.ok) throw new Error(`DST: ${res.status} ${await res.text().then((t) => t.slice(0, 200))}`);
...
/** Shift a "YYYYQn" label by n quarters. Used for the year-on-year delta. */
function shiftQuarters(period: string, delta: number): string {
  const m = /^(\d{4})Q([1-4])$/.exec(period);
  if (!m) return period;
  const total = Number(m[1]) * 4 + (Number(m[2]) - 1) + delta;
